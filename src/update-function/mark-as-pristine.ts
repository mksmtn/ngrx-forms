 ...
 */
export function markAsPristine<TValue extends FormControlValueTypes>(
 ...
 */
export function markAsPristine<TValue>(
 ...
 */
export function markAsPristine<TValue extends KeyValue>(
...
export function markAsPristine<TValue>(state: AbstractControlState<TValue>) {
  return abstractControlReducer(
    state,
    markAsPristineAction({ controlId: state.id })
  );
}
