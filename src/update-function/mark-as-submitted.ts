 ...
 */
export function markAsSubmitted<TValue extends FormControlValueTypes>(
 ...
 */
export function markAsSubmitted<TValue>(
 ...
 */
export function markAsSubmitted<TValue extends KeyValue>(
...
export function markAsSubmitted<TValue>(state: AbstractControlState<TValue>) {
  return abstractControlReducer(
    state,
    markAsSubmittedAction({ controlId: state.id })
  );
}
