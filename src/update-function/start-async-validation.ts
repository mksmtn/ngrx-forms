 ...
 */
export function startAsyncValidation<TValue>(
  state: AbstractControlState<TValue>,
  name: string
): FormState<TValue>;

export function startAsyncValidation<TValue>(
  nameOrState: string | AbstractControlState<TValue>,
  name?: string
) {
  if (isFormState(nameOrState)) {
    return abstractControlReducer(
      nameOrState,
      startAsyncValidationAction({ controlId: nameOrState.id, name: name! })
    );
  }

  return (s: AbstractControlState<TValue>) =>
    startAsyncValidation(ensureState(s), nameOrState);
}
