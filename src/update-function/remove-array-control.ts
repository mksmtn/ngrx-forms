 ...
 */
export function removeArrayControl(
  index: number
): <TValue>(state: FormArrayState<TValue>) => FormArrayState<TValue>;
 ...
 */
export function removeArrayControl<TValue>(
  state: FormArrayState<TValue>,
  index: number
): FormArrayState<TValue>;

export function removeArrayControl<TValue>(
  indexOrState: number | FormArrayState<TValue>,
  index?: number
) {
  if (isArrayState(indexOrState)) {
    return formArrayReducer(
      indexOrState,
      removeArrayControlAction({ controlId: indexOrState.id, index: index! })
    );
  }

  return (s: FormArrayState<TValue>) =>
    removeArrayControl(ensureState(s), indexOrState as number);
}
