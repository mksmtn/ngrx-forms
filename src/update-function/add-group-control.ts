 ...
 */
export function addGroupControl<
  TValue extends KeyValue,
  TControlKey extends keyof TValue = keyof TValue
>(
  name: TControlKey,
...
>(
  ...
  if (isGroupState(nameOrState)) {
    return formGroupReducer(
      ...
      addGroupControlAction({
        ...
        // todo: better typing?
        name: valueOrName as keyof KeyValue,
        value: value!,
      })
    );
  }
  ...
  return (s: FormGroupState<TValue>) =>
    addGroupControl(
      ensureState(s),
      nameOrState as TControlKey,
      valueOrName as TValue[TControlKey]
    );
}
