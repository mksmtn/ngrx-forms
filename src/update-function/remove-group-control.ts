 ...
 */
export function removeGroupControl<TValue extends KeyValue>(
  name: keyof TValue
...
export function removeGroupControl<TValue extends KeyValue>(
  nameOrState: keyof TValue | FormGroupState<TValue>,
  name?: keyof TValue
) {
  if (isGroupState(nameOrState)) {
    return formGroupReducer(
      ...
      removeGroupControlAction({
        ...
        name: name as keyof KeyValue,
      })
    );
  }

  return (s: FormGroupState<TValue>) =>
    removeGroupControl(ensureState(s), nameOrState as keyof TValue);
}
