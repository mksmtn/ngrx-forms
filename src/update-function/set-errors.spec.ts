...
describe(setErrors.name, () => {
  ...
  it("should merge single errors with rest param array errors", () => {
    ...
    const errors2 = { min: 1 };
    const mergedErrors = { required: { actual: undefined }, min: 1 };
    const resultState = setErrors(errors1, [errors2])(
      INITIAL_STATE.controls.inner
    );
    expect(resultState.errors).toEqual(mergedErrors);
  });
  ...
  it("should merge single errors with rest param array errors uncurried", () => {
    ...
    const errors2 = { min: 1 };
    const mergedErrors = { required: { actual: undefined }, min: 1 };
    const resultState = setErrors(INITIAL_STATE.controls.inner, errors1, [
      errors2,
    ]);
    expect(resultState.errors).toEqual(mergedErrors);
  });
  ...
  it("should merge errors from array of errors uncurried", () => {
    ...
    const errors2 = { min: 1 };
    const mergedErrors = { required: { actual: undefined }, min: 1 };
    const resultState = setErrors(INITIAL_STATE.controls.inner, [
      errors1,
      errors2,
    ]);
    expect(resultState.errors).toEqual(mergedErrors);
