...
describe(`form group ${setUserDefinedPropertyReducer.name}`, () => {
  it("should skip any actionof the wrong type", () =>
    expect(
      setUserDefinedPropertyReducer(INITIAL_STATE, { type: "" } as any)
    ).toBe(INITIAL_STATE));

  it("should update state user defined properties if different", () => {
    const prop = "prop";
    const value = 12;
    const resultState = setUserDefinedPropertyReducer(
      INITIAL_STATE,
  ...
  it("should not update state user defined properties if same", () => {
    ...
    const state = {
      ...INITIAL_STATE,
      userDefinedProperties: { [prop]: value },
    };
    const resultState = setUserDefinedPropertyReducer<typeof state.value>(
      state,
      setUserDefinedPropertyAction({
        controlId: FORM_CONTROL_ID,
        name: prop,
