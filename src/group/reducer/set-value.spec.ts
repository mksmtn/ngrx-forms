...
describe(`form group ${setValueReducer.name}`, () => {
  it("should update state value if different", () => {
    const value = { inner: "A" };
    const resultState = setValueReducer(
      INITIAL_STATE,
  ...
  it("should create child states on demand", () => {
    ...
    const resultState = setValueReducer<FormGroupValue>(
      INITIAL_STATE,
      setValueAction({ controlId: FORM_CONTROL_ID, value })
    );
    expect(resultState.value).toEqual(value);
  ...
  it("should remove child states on demand", () => {
    ...
    resultState = setValueReducer(
      resultState,
      setValueAction({ controlId: FORM_CONTROL_ID, value: value2 })
    );
  ...
  it("should remove child states on demand when value is empty", () => {
    ...
    const resultState = setValueReducer<FormValue>(
      state,
      setValueAction({ controlId: id, value: {} })
    );
  ...
  it("should aggregate child values for group children", () => {
    let resultState = setValueReducer<FormGroupValue>(
      ...
      setValueAction({
        ...
        value: { inner: "A", inner3: { inner4: "C" } },
      })
    );
    const value = { inner4: "D" };
    resultState = setValueReducer(
      resultState,
      setValueAction({ controlId: FORM_CONTROL_INNER3_ID, value })
    );
  ...
  it("should aggregate child values for array children", () => {
    let resultState = setValueReducer<FormGroupValue>(
      ...
      setValueAction({
        controlId: FORM_CONTROL_ID,
        value: { inner: "A", inner5: ["C"] },
      })
    );
    const value = ["D"];
    resultState = setValueReducer(
      resultState,
      setValueAction({ controlId: FORM_CONTROL_INNER5_ID, value })
    );
  ...
  it("should aggregate nested child values in groups", () => {
    ...
    resultState = setValueReducer(
      resultState,
      setValueAction({ controlId: FORM_CONTROL_INNER4_ID, value })
    );
  ...
  it("should aggregate nested child values in arrays", () => {
    ...
    resultState = setValueReducer(
      resultState,
      setValueAction({ controlId: FORM_CONTROL_INNER5_0_ID, value })
    );
