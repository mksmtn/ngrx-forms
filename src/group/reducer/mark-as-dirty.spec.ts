...
describe(`form group ${markAsDirtyReducer.name}`, () => {
  const INITIAL_STATE_FULL_DIRTY = setPropertiesRecursively(
    INITIAL_STATE_FULL,
    [
      ["isDirty", true],
      ["isPristine", false],
  ...
  it("should mark itself and all children recursively as dirty", () => {
    const resultState = markAsDirtyReducer(
      INITIAL_STATE_FULL,
  ...
  it("should forward actions to children", () => {
    const resultState = markAsDirtyReducer(
      INITIAL_STATE,
      markAsDirtyAction({ controlId: INITIAL_STATE.controls.inner.id })
    );
