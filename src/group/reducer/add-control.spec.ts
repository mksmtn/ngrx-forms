...
describe(`form group ${addControlReducer.name}`, () => {
  it("should create child state for control child", () => {
    ...
    const resultState = addControlReducer<FormGroupValue>(
      INITIAL_STATE,
      action
    );
  ...
  it("should throw if trying to add existing control", () => {
    ...
    expect(() =>
      addControlReducer<FormGroupValue>(INITIAL_STATE, action)
    ).toThrowError();
  });

  it("should forward actions to children", () => {
    const state = createFormGroupState<{ inner: { inner2?: string } }>(
      FORM_CONTROL_ID,
      { inner: {} }
    );
    const value = "B";
    const action = addGroupControlAction({
      controlId: state.controls.inner.id,
      name: "inner2",
      value,
    });
