...
describe(`form group ${setAsyncErrorReducer.name}`, () => {
  it("should skip any action of the wrong type", () =>
    expect(setAsyncErrorReducer(INITIAL_STATE, { type: "" } as any)).toBe(
      INITIAL_STATE
    ));
  ...
  it("should update state with error", () => {
    ...
    const resultState = setAsyncErrorReducer<typeof state.value>(
      state,
  ...
  it("should aggregate child errors", () => {
    ...
    const resultState = setAsyncErrorReducer(
      state,
      setAsyncErrorAction({ controlId: FORM_CONTROL_INNER_ID, name, value })
    );
  ...
  it("should aggregate child errors for group children", () => {
    ...
    const resultState = setAsyncErrorReducer(
      state,
      setAsyncErrorAction({ controlId: FORM_CONTROL_INNER3_ID, name, value })
    );
  ...
  it("should aggregate child errors for array children", () => {
    ...
    const resultState = setAsyncErrorReducer(
      state,
      setAsyncErrorAction({ controlId: FORM_CONTROL_INNER5_ID, name, value })
    );
  ...
  it("should aggregate multiple child errors", () => {
    ...
    let resultState = setAsyncErrorReducer(
      state,
      setAsyncErrorAction({
        controlId: FORM_CONTROL_INNER_ID,
        name: name1,
        value: value1,
      })
    );
    resultState = setAsyncErrorReducer(
      resultState,
      setAsyncErrorAction({
        controlId: FORM_CONTROL_INNER3_ID,
        name: name2,
        value: value2,
      })
    );
