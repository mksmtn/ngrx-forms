...
describe(`form array ${addControlReducer.name}`, () => {
  ...
  it("should create child state for group child", () => {
    ...
    expect(resultState.value).toEqual([
      ...INITIAL_STATE_NESTED_GROUP.value,
      value,
    ]);
  ...
  it("should create child state for array child", () => {
    ...
    expect(resultState.value).toEqual([
      ...INITIAL_STATE_NESTED_ARRAY.value,
      value,
    ]);
    expect(resultState.controls[2].value).toEqual(value);
    expect(resultState.controls[2].controls).toBeDefined();
  ...
  it("should update deeply nested child IDs after an insertion", () => {
    ...
    const resultState = addControlReducer<DeeplyNestedGroupFormValue>(
      INITIAL_DEEPLY_NESTED_GROUPS_FORM_STATE,
      action
    );
    ...
    resultState.controls.forEach((control, index) => {
      ...
      expect(control.controls.deep.id).toEqual(
        `${FORM_CONTROL_ID}.${index}.deep`
      );
      expect(control.controls.deep.controls.inners.id).toEqual(
        `${FORM_CONTROL_ID}.${index}.deep.inners`
      );
      control.controls.deep.controls.inners.controls.forEach(
        (deepControl, deepIndex) => {
          expect(deepControl.id).toEqual(
            `${FORM_CONTROL_ID}.${index}.deep.inners.${deepIndex}`
          );
          expect(deepControl.controls.inner.id).toEqual(
            `${FORM_CONTROL_ID}.${index}.deep.inners.${deepIndex}.inner`
          );
        }
      );
    });
