...
describe(`form array ${markAsPristineReducer.name}`, () => {
  it("should update state if dirty", () => {
    ...
    const resultState = markAsPristineReducer(
      state,
  ...
  it("should mark group children as pristine", () => {
    const state = setPropertiesRecursively(INITIAL_STATE_NESTED_GROUP, [
      ["isDirty", true],
      ["isPristine", false],
    ]);
    const resultState = markAsPristineReducer(
  ...
  it("should mark array children as pristine", () => {
    const state = setPropertiesRecursively(INITIAL_STATE_NESTED_ARRAY, [
      ["isDirty", true],
      ["isPristine", false],
    ]);
    const resultState = markAsPristineReducer(
  ...
  it("should forward actions to children", () => {
    ...
    const resultState = markAsPristineReducer(
      state,
      markAsPristineAction({ controlId: state.controls[0].id })
    );
  ...
  it("should update state if dirty and empty", () => {
    const state = {
      ...createFormArrayState(FORM_CONTROL_ID, []),
      isDirty: true,
      isPristine: false,
    };
    const resultState = markAsPristineReducer(
