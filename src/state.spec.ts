...
describe("state", () => {
  ...
  describe("group", () => {
    ...
    const INITIAL_STATE = createFormGroupState<typeof INITIAL_VALUE>(
      FORM_CONTROL_ID,
      INITIAL_VALUE
    );
    ...
    it("should produce the same state as is computed after an action is applied", () => {
      ...
      expect(state.pendingValidations).toEqual(
        INITIAL_STATE.pendingValidations
      );
      ...
      expect(state.userDefinedProperties).toEqual(
        INITIAL_STATE.userDefinedProperties
      );
    });
  ...
  describe("array", () => {
    ...
    const INITIAL_STATE = createFormArrayState<string>(
      FORM_CONTROL_ID,
      INITIAL_VALUE
    );
    ...
    it("should create group child", () => {
      ...
      const initialState = createFormArrayState<{ control: string }>(
        FORM_CONTROL_ID,
        initialValue
      );
    ...
    it("should create array child", () => {
      ...
      const initialState = createFormArrayState<string[]>(
        FORM_CONTROL_ID,
        initialValue
      );
      expect(initialState.controls[0].value).toEqual(initialValue[0]);
    ...
    it("should create mixed children", () => {
      ...
      const initialState = createFormArrayState<string[] | { control: string }>(
        FORM_CONTROL_ID,
        initialValue
      );
      expect(initialState.controls[0].value).toEqual(initialValue[0]);
  ...
  describe(verifyFormControlValueIsValid.name, () => {
    it("should return valid control values unmodified", () => {
      ...
      expect(verifyFormControlValueIsValid(undefinedValue)).toBe(
        undefinedValue
      );
    });
    ...
    it("should return boxed serializable values unmodified", () => {
      ...
      expect(verifyFormControlValueIsValid(boxedStringValue)).toBe(
        boxedStringValue
      );
      expect(verifyFormControlValueIsValid(boxedNumberValue)).toBe(
        boxedNumberValue
      );
      expect(verifyFormControlValueIsValid(boxedBooleanValue)).toBe(
        boxedBooleanValue
      );
      expect(verifyFormControlValueIsValid(boxedObjectValue)).toBe(
        boxedObjectValue
      );
      expect(verifyFormControlValueIsValid(boxedArrayValue)).toBe(
        boxedArrayValue
      );
      expect(verifyFormControlValueIsValid(boxedNullValue)).toBe(
        boxedNullValue
      );
      expect(verifyFormControlValueIsValid(boxedUndefinedValue)).toBe(
        boxedUndefinedValue
      );
    });
    ...
    it("should return boxed serializable values with undefined properties", () => {
      ...
      expect(verifyFormControlValueIsValid(boxedUndefinedObjectValue)).toEqual(
        boxedUndefinedObjectValue
      );

      const boxedSubUndefinedObjectValue = box({ v: "A", u: { u: undefined } });
      expect(
        verifyFormControlValueIsValid(boxedSubUndefinedObjectValue)
      ).toEqual(boxedSubUndefinedObjectValue);
    });
    ...
    it("should throw for non-serializable boxed values", () => {
      ...
      expect(() =>
        verifyFormControlValueIsValid(boxedFunctionValue)
      ).toThrowError();
      expect(() =>
        verifyFormControlValueIsValid(boxedObjectWithFunctionValue)
      ).toThrowError();
    });
  });
  ...
  describe(isArrayState.name, () => {
    it("should return true for array state", () => {
      const INITIAL_STATE = createFormArrayState<string>(FORM_CONTROL_ID, [
        "abc",
      ]);
    ...
    it("should return false for group state", () => {
      const INITIAL_STATE = createFormGroupState<any>(FORM_CONTROL_ID, {
        control: "abc",
      });
  ...
  describe(computeArrayState.name, () => {
    ...
    it("should aggregate child values", () => {
      const state = computeArrayState<string>(
        FORM_CONTROL_ID,
        CONTROLS,
        [],
        {},
    ...
    it("should aggregate child errors", () => {
      const childError = { required: { actual: undefined } };
    ...
    it("should mark as enabled if all children are disabled and state was enabled", () => {
      ...
      const state = computeArrayState(
        FORM_CONTROL_ID,
        [disabledControl1, disabledControl2],
        [],
        {},
        [],
        {},
    ...
    it("should mark as validation pending if child has pending validations", () => {
      ...
      const state = computeArrayState<string>(
        FORM_CONTROL_ID,
        [validationPendingControl, CONTROL_2],
        [],
        {},
        [],
        {},
  ...
  describe(computeGroupState.name, () => {
    ...
    const CONTROL_1 = createFormControlState(
      FORM_CONTROL_INNER_ID,
      VALUE.inner1
    );
    const CONTROL_2 = createFormControlState(
      FORM_CONTROL_INNER2_ID,
      VALUE.inner2
    );
    ...
    it("should aggregate child values", () => {
      const state = computeGroupState<typeof VALUE>(
        FORM_CONTROL_ID,
        CONTROLS,
        { inner1: "", inner2: "" },
        {},
    ...
    it("should aggregate child errors", () => {
      ...
      const state = computeGroupState<typeof VALUE>(
        FORM_CONTROL_ID,
        { inner1: controlWithError, inner2: CONTROL_2 },
        VALUE,
        {},
        [],
        {},
    ...
    it("should mark as dirty if some child is dirty and state was not dirty", () => {
      ...
      const state = computeGroupState(
        FORM_CONTROL_ID,
        { inner1: dirtyControl, inner2: CONTROL_2 },
        VALUE,
        {},
        [],
        {},
    ...
    it("should mark as enabled if some child is enabled and state was enabled", () => {
      ...
      const state = computeGroupState(
        FORM_CONTROL_ID,
        { inner1: disabledControl, inner2: CONTROL_2 },
        VALUE,
        {},
        [],
        {},
    ...
    it("should mark as enabled if all children are disabled and state was enabled", () => {
      ...
      const disabledControls = {
        inner1: disabledControl1,
        inner2: disabledControl2,
      };
      const state = computeGroupState(
        FORM_CONTROL_ID,
        disabledControls,
    ...
    it("should mark as touched if some child is touched and state was not touched", () => {
      ...
      const state = computeGroupState(
        FORM_CONTROL_ID,
        { inner1: touchedControl, inner2: CONTROL_2 },
        VALUE,
        {},
        [],
        {},
    ...
    it("should mark as submitted if some child is submitted and state was not submitted", () => {
      ...
      const state = computeGroupState(
        FORM_CONTROL_ID,
        { inner1: submittedControl, inner2: CONTROL_2 },
        VALUE,
        {},
        [],
        {},
    ...
    it("should mark as validation pending if child has pending validations", () => {
      ...
      const state = computeGroupState(
        FORM_CONTROL_ID,
        { inner1: validationPendingControl, inner2: CONTROL_2 },
        VALUE,
        {},
        [],
        {},
