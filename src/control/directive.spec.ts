...
describe(NgrxFormControlDirective.name, () => {
  ...
  beforeEach(() => {
    ...
    directive = new NgrxFormControlDirective<string | null>(
      elementRef,
      document,
      actionsSubject as any,
      [viewAdapter],
      []
    );
  ...
  describe("writing values and dispatching value and dirty actions", () => {
    ...
    it("should write the value when the state value does not change but the id does", () => {
      const spy = spyOn(viewAdapter, "setViewValue");
      directive.ngrxFormControlState = {
        ...INITIAL_STATE,
        id: `${FORM_CONTROL_ID}1`,
      };
    ...
    it(`should dispatch a ${setValueAction.name} if the view value changes`, (done) => {
      ...
      actions$.pipe(first()).subscribe((a) => {
        expect(a).toEqual(
          setValueAction({ controlId: INITIAL_STATE.id, value: newValue })
        );
        done();
      });
  ...
  describe("value conversion", () => {
    ...
    it("should convert the view value if it changes", (done) => {
      actions$.pipe(first()).subscribe((a) => {
        expect(a).toEqual(
          setValueAction({ controlId: INITIAL_STATE.id, value: STATE_VALUE })
        );
        done();
      });
  ...
  describe("focus tracking", () => {
    describe("is enabled", () => {
      ...
      it(`should dispatch a ${focusAction.name} when element becomes focused and state is not focused`, (done) => {
        directive.ngOnInit();
      ...
      it(`should dispatch an ${unfocusAction.name} when element becomes unfocused and state is focused`, (done) => {
        directive.ngOnInit();
  ...
  describe("ControlValueAccessor integration", () => {
    it("should adapt a control value accessor to a form view adapter if no form view adapter is provided", () => {
      const controlValueAccessor: ControlValueAccessor = jasmine.createSpyObj(
      ...
      directive.ngOnInit();
      expect(controlValueAccessor.writeValue).toHaveBeenCalledWith(
        INITIAL_STATE.value
      );
    ...
    it("should throw if more than one control value accessor is provided", () => {
      expect(
        () =>
          new NgrxFormControlDirective<string | null>(
            elementRef,
            document,
            actionsSubject as any,
            [],
