...
@Component({
  ...
  selector: "radio-test",
  template:
    '<input *ngFor="let o of options; trackBy: trackByIndex" type="radio" [value]="o" [ngrxFormControlState]="state" />',
})
...
describe(RadioTestComponent.name, () => {
  ...
  beforeEach(() => {
    ...
    element1 = (fixture.nativeElement as HTMLElement).querySelectorAll(
      "input"
    )[0];
    element2 = (fixture.nativeElement as HTMLElement).querySelectorAll(
      "input"
    )[1];
  });
  ...
  it(`should trigger a ${setValueAction.name} with the selected value when an option is selected`, (done) => {
    actions$.pipe(take(1)).subscribe((a) => {
      ...
      expect((a as ReturnType<typeof setValueAction>).value).toBe(
        RADIO_OPTIONS[0]
      );
      done();
    });
  ...
  it(`should trigger ${setValueAction.name}s and ${markAsDirtyAction.name}s when switching between options`, (done) => {
    actions$.pipe(bufferCount(4), take(1)).subscribe(([a1, a2, a3, a4]) => {
      ...
      expect((a1 as ReturnType<typeof setValueAction>).value).toBe(
        RADIO_OPTIONS[0]
      );
      expect((a3 as ReturnType<typeof setValueAction>).value).toBe(
        RADIO_OPTIONS[1]
      );
      done();
    });
  ...
  it(`should trigger a ${setValueAction.name} if the value of the selected option changes`, (done) => {
    ...
    actions$.pipe(take(1)).subscribe((a) => {
      expect(a.type).toBe(setValueAction.type);
      expect((a as ReturnType<typeof setValueAction>).value).toBe(newValue);
      done();
    });
