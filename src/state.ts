...
export type FormControlValueTypes =
  | Boxed<any>
  | string
  | number
  | boolean
 ...
 */
export interface FormControlState<TValue extends FormControlValueTypes>
  extends AbstractControlState<TValue> {
  /**
 ...
 */
export interface FormGroupState<TValue extends KeyValue>
  extends AbstractControlState<TValue> {
  /**
 ...
 */
export interface FormArrayState<TValue>
  extends AbstractControlState<readonly TValue[]> {
  /**
...
export type InferredBoxedFormState<T extends InferenceWrapper<any>> =
  T extends InferenceWrapper<Boxed<infer U>>
...
export type InferredStringFormState<T extends InferenceWrapper<any>> =
  T extends InferenceWrapper<string>
...
export type InferredNumberFormState<T extends InferenceWrapper<any>> =
  T extends InferenceWrapper<number>
...
export type InferredBooleanFormState<T extends InferenceWrapper<any>> =
  T extends InferenceWrapper<boolean>
 ...
 */
export function isFormState<TValue = any>(
  state: any
): state is FormState<TValue> {
  return (
    !!state &&
 ...
 */
export function isArrayState<TValue = any>(
  state: any
): state is FormArrayState<TValue> {
  return (
    isFormState(state) &&
 ...
 */
export function isGroupState<TValue extends KeyValue = any>(
  state: any
): state is FormGroupState<TValue> {
  return (
    isFormState(state) &&
    state.hasOwnProperty("controls") &&
    !Array.isArray((state as any).controls) &&
...
export function createChildState<TValue>(
  id: string,
  childValue: TValue
): FormState<TValue> {
...
export function verifyFormControlValueIsValid<TValue>(value: TValue): TValue {
  if (
    value === null ||
  ...
  if (!isBoxed(value)) {
    const errorMsg =
      "Form control states only support undefined, null, string, number, and boolean values as well as boxed values";
    throw new Error(
      `${errorMsg}; got ${JSON.stringify(value)} of type ${typeof value}`
    ); // `;
  }
  ...
  if (
    deepEquals(value, deserialized, { treatUndefinedAndMissingKeyAsSame: true })
  ) {
    return value;
  }

  throw new Error(
    `A form control value must be serializable (i.e. value === JSON.parse(JSON.stringify(value))), got: ${JSON.stringify(
      value
    )}`
  );
}
...
export function getFormGroupValue<TValue extends KeyValue>(
  ...
  let hasChanged =
    Object.keys(originalValue).length !== Object.keys(controls).length;
...
export function getFormGroupErrors<TValue extends KeyValue>(
  ...
  const groupErrors = Object.keys(originalErrors)
    ...
    .reduce(
      (res, key) => Object.assign(res, { [key]: originalErrors[key] }),
      {} as ValidationErrors
    );
...
export function computeGroupState<TValue extends KeyValue>(
  ...
  const typedControls = Object.keys(controls).map(
    (key) => controls[key] as AbstractControlState<TValue[keyof TValue]>
  );
  const isDirty =
    flags.wasOrShouldBeDirty ||
    typedControls.some((control) => control.isDirty);
  const isEnabled =
    flags.wasOrShouldBeEnabled ||
    typedControls.some((control) => control.isEnabled);
  const isTouched =
    flags.wasOrShouldBeTouched ||
    typedControls.some((control) => control.isTouched);
  const isSubmitted =
    flags.wasOrShouldBeSubmitted ||
  ...
  const controls = Object.keys(initialValue)
    .map(
      (key: keyof TValue) =>
        [
          key,
          createChildState(`${id}.${key as string}`, initialValue[key]),
        ] as [string, FormState<any>]
    )
    .reduce(
      (res, [controlId, state]) => Object.assign(res, { [controlId]: state }),
...
export function computeArrayState<TValue>(
  ...
  const isValid = isEmpty(errors);
  const isDirty =
    flags.wasOrShouldBeDirty || controls.some((state) => state.isDirty);
  const isEnabled =
    flags.wasOrShouldBeEnabled || controls.some((state) => state.isEnabled);
  const isTouched =
    flags.wasOrShouldBeTouched || controls.some((state) => state.isTouched);
  const isSubmitted =
    flags.wasOrShouldBeSubmitted || controls.some((state) => state.isSubmitted);
  const isValidationPending =
    pendingValidations.length > 0 ||
...
export function createFormArrayState<TValue>(
  ...
  const controls = initialValue.map((value, i) =>
    createChildState(`${id}.${i}`, value)
  );
